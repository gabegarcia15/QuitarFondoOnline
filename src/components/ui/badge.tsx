...
export function Badge({ color = 'zinc', className, ...props }: BadgeProps & React.ComponentPropsWithoutRef<'span'>) {
  return (
    <span
      ...
      className={clsx(
        className,
        'inline-flex items-center gap-x-1.5 px-2 py-0.5 text-[0.6875rem] font-semibold uppercase tracking-wider forced-colors:outline',
        colors[color]
      )}
    />
  )
...
export const BadgeButton = forwardRef(function BadgeButton(
  ...
  const classes = clsx(
    className,
    'group relative inline-flex rounded-md focus:not-data-focus:outline-hidden data-focus:outline-2 data-focus:outline-offset-2 data-focus:outline-red-500'
  )
