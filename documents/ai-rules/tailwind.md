...
**CRITICAL**: Tailwind v4 requires the `@tailwindcss/postcss` plugin for Next.js integration. Without it, styles will not be applied.
...
### Required postcss.config.mjs (at project root)
...
### Required globals.css setup

```css
@import "tailwindcss";

/* Your custom styles and @theme extensions here */
```

**Note**: Tailwind v4 uses CSS-first configuration. There is NO `tailwind.config.js` file - all customization is done in CSS via `@theme` blocks.
